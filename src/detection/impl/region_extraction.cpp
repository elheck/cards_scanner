...
// Located below the collector number line
constexpr double set_left_ratio = 0.04;  // 4% from left
constexpr double set_top_ratio = 0.955;  // 95.5% from top (adjusted)
constexpr double set_width_ratio = 0.12; // 12% of card width
constexpr double set_height_ratio =
    0.035; // 3.5% of card height (larger region)
...
cv::Rect extractArtRegionRegular(const cv::Mat &image) {
  ...
  for (const auto &contour : contours) {
    ...
    if (approx.size() == 4 && cv::isContourConvex(approx)) {
      ...
      // Filter based on aspect ratio (artwork boxes are roughly square or
      // rectangular)
