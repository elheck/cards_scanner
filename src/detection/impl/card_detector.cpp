...
constexpr double min_card_area_ratio =
    0.1; // Card must be at least 10% of image
...
constexpr int thresh_ratio =
    20; // Divisor for calculating threshold kernel size
...
bool loadImage(const std::filesystem::path &imagePath, cv::Mat &originalImage,
  ...
  if (originalImage.empty()) {
    spdlog::error(
        "Failed to load image (format not recognized or file corrupted): {}",
        imagePath.string());
...
cv::Mat warpCard(const std::vector<cv::Point2f> &corners,
  ...
  std::vector<cv::Point2f> dst_points{
      ...
      cv::Point2f(static_cast<float>(normalizedWidth),
...
bool detectCards(const cv::Mat &undistortedImage,
  ...
  int blur_radius =
      ((min_dim / blur_ratio + 1) / 2) * 2 + 1; // Round to nearest odd
  int dilate_radius = static_cast<int>(
      std::floor(static_cast<double>(min_dim) / dilate_ratio + dilate_round));
  int thresh_radius =
      ((min_dim / thresh_ratio + 1) / 2) * 2 + 1; // Round to nearest odd
  ...
  cv::GaussianBlur(blurred, blurred,
                   cv::Size(gaussian_kernel_size, gaussian_kernel_size), 0);

  // Apply adaptive threshold with adjusted parameters
  cv::Mat binary;
  cv::adaptiveThreshold(blurred, binary, max_pixel_value,
                        cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY_INV,
                        thresh_radius, thresh_c_value);

  // Create kernel for morphological operations
  cv::Mat kernel = cv::getStructuringElement(
      cv::MORPH_RECT, cv::Size(dilate_radius, dilate_radius));
  ...
  for (const auto &contour : contours) {
    ...
    double aspect_ratio =
        static_cast<double>(bound_rect.width) / bound_rect.height;
  ...
  auto max_contour = std::max_element(
      valid_contours.begin(), valid_contours.end(),
