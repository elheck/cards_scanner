...
struct CardInfo {
    ...
    std::string setName;          // Full set name (e.g., "Duskmourn: House of Horror Commander")
...
public:
    ...
    [[nodiscard]] std::optional<CardInfo> getCardByCollectorNumber(
    ...
    [[nodiscard]] std::optional<CardInfo> getCardByFuzzyName(
        const std::string& name) const;

    /// Search for cards matching a query
    /// Example: searchCards("set:dsc type:artifact")
    [[nodiscard]] std::vector<CardInfo> searchCards(
